use crate::common::{Animation, BufferTarget, BufferTargetRgb888, HEIGHT, WIDTH};
use alloc::{
    boxed::Box,
...
#[derive(Clone)]
pub struct Blank {}
impl_animation_clone!(Blank);

impl Animation for Blank {
    fn draw(&mut self, _ms_since_start: u32, target: &mut BufferTargetRgb888) -> () {
        target.buffer.fill(Rgb888::new(0, 0, 0x10));
    }
}

#[derive(Clone)]
pub struct TextMessage {}
impl_animation_clone!(TextMessage);

impl Animation for TextMessage {
    fn draw(&mut self, _ms_since_start: u32, target: &mut BufferTargetRgb888) -> () {
...
impl_animation_clone!(ScrollingMessage);

impl Animation for ScrollingMessage {
    ...
    //             &self.msg,
    //             Point::new(
    //                 -((ms_since_start as i32 / 10) % (self.msg.len() as i32 * 10)),
    //                 (HEIGHT as i32 - 20) / 2,
    //             ),
    //             style,
    //             TextStyleBuilder::new()
    ...
    fn draw(&mut self, ms_since_start: u32, target: &mut BufferTargetRgb888) -> () {
        ...
        let scroll_offset = (t * 50.0) as i32 % (self.msg.len() as i32 * 10); // 50 px/sec
        let x = -scroll_offset;
...
#[derive(Clone)]
pub struct Squares {}
impl_animation_clone!(Squares);

impl Animation for Squares {
    fn draw(&mut self, _ms_since_start: u32, target: &mut BufferTargetRgb888) -> () {
        target.buffer[0] = Rgb888::new(0, 0xFF, 0);
    }
}

#[derive(Clone)]
pub struct Circles {}
impl_animation_clone!(Circles);

impl Animation for Circles {
    fn draw(&mut self, ms_since_start: u32, target: &mut BufferTargetRgb888) -> () {
...
#[derive(Clone)]
pub struct Rainbows {}
impl_animation_clone!(Rainbows);

impl Animation for Rainbows {
    fn draw(&mut self, ms_since_start: u32, target: &mut BufferTargetRgb888) {
...
impl_animation_clone!(TestPattern);

impl Animation for TestPattern {
    fn draw(&mut self, ms_since_start: u32, target: &mut BufferTargetRgb888) {
...
// impl Animation for Xp {
//     fn draw(&mut self, ms_since_start: u32, target: &mut BufferTargetRgb888) -> () {
//         let mut buffer565 = BufferTargetRgb565::new();

//         self.scene.render(&mut buffer565, ms_since_start);

//         let mut i = 0;
//         for c in buffer565.buffer {
//             let r = ((c.r() as u16) << 3) | ((c.r() as u16) >> 2); // 5->8
//             let g = ((c.g() as u16) << 2) | ((c.g() as u16) >> 4); // 6->8
//             let b = ((c.b() as u16) << 3) | ((c.b() as u16) >> 2); // 5->8
//             target.buffer[i] = Rgb888::new(r as u8, g as u8, b as u8);
//             i += 1;
//         }
//     }
// }
...
impl_animation_clone!(RainbowWheel);
...
impl_animation_clone!(SpinningCube);

impl Animation for SpinningCube {
    fn draw(&mut self, ms_since_start: u32, target: &mut BufferTargetRgb888) {
...
#[derive(Clone)]
pub struct Columns {}
impl_animation_clone!(Columns);

impl Animation for Columns {
    fn draw(&mut self, ms_since_start: u32, target: &mut BufferTargetRgb888) {
        for y in 0..HEIGHT {
            for x in 0..WIDTH {
                if x % 8 == 0 {
...
// pub fn get_animation(index: usize) -> Box<dyn Animation> {
//     match index {
//         0 => Box::new(Blank {}),
//         1 => Box::new(Circles {}),
//         2 => Box::new(Squares {}),
...
//         5 => Box::new(TestPattern {}),
//         6 => Box::new(SpinningCube {}),
//         7 => Box::new(Columns {}),
//         _ => Box::new(Blank {}),
//     }
// }
